...
@Config
@TeleOp(group = "dev")
public class NewLinearTest extends LinearOpMode {
...
//        drive.followTrajectory(desired_x, desired_y, Math.toRadians(deg_desired_heading));
        LinearTraj linear = new LinearTraj(drive.getPoseEstimate(), new Pose2d(desired_x, desired_y, Math.toRadians(deg_desired_heading)));
        drive.followTrajectory(linear);
    }
